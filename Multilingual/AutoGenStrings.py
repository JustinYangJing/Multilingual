...
def SearchFile(path,suffixs):
    files=os.listdir(path)
    list = []
    for f in files:
        fl= os.path.join(path,f)
        if os.path.isdir(fl):
            list.extend(SearchFile(fl,suffixs))
        elif os.path.isfile(fl):
            for suffix in suffixs:
                if os.path.splitext(fl)[1] == suffix:
                    list.append(fl)
                    break
    return list
...
def extractFilePrefix(file_path):
	...
	#将绝对路径中的最后一级目录去掉 如xxx/yyy/aaa.c 变为xxx/aaa
def extractFilePath(file_path):
	seg = file_path.split('/')
	lastindex = len(seg) - 1
	prefix =  seg[lastindex].split('.')[0]
	if len(seg) > 1:
...
#	sourceFile_list = glob.glob(sourceFilePath)
	sourceFile_list = SearchFile(filePath,['.storyboard','.xib'])
...
#	targetFile_list = glob.glob(targetFilePath)
	targetFile_list = SearchFile(filePath,['.strings'])
	...
	for sourcePath in sourceFile_list:
		sourceprefix = extractFilePath(sourcePath)
		...
		if generateStoryboardStringsfile(sourcePath,tempFile_Path) == 1:
			...
			for targetPath in targetFile_list:
				targetprefix = extractFilePath(targetPath)
